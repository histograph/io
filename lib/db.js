...
function datasetExists(req, res, next) {
  graphDb.cypher({
    query: 'MATCH (dataset:Dataset {datasetid: {id}}) RETURN dataset',
    params: {
...
function updateDataset(res, dataset, callback) {
  ...
  graphDb.cypher({
    query: 'MERGE (s:Dataset { datasetid: {id} }) ON MATCH SET ' + queryDataset,
    params: params
  }, function(err) {
    if (err) {
      send500(res, err.neo4j);
...
function deleteDataset(res, datasetId, callback) {
  graphDb.cypher({
    query: 'MATCH (s:Dataset { datasetid: {id}})-[r:OWNS]-() DELETE s, r',
    params: {
      id: datasetId
    }
  }, function(err) {
...
function getOwnerForDataset(res, dataset, callback) {
  graphDb.cypher({
    query: 'MATCH (owner:Owner)-[:OWNS]-(:Dataset {datasetid: {dataset}}) RETURN owner',
    params: {
      dataset: dataset
    }
  }, function(err, results) {
