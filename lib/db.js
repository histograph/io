...
const esIndex = require('./elasticsearch-index');
...
if (config.neo4j.user && config.neo4j.password) {
  const neo4jUrl = 'http://' + config.neo4j.user + ':' + config.neo4j.password + '@' +
    config.neo4j.host + ':' + config.neo4j.port;
} else {
  const neo4jUrl = 'http://' + config.neo4j.host + ':' + config.neo4j.port;
}
...
module.exports.initAdmin = function() {
  ...
  graphDb.cypher({
    ...
    if (err) {
      my_log.error('Error creating Neo4j admin owner');
    }
  });
};
